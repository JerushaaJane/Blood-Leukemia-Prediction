...
def training():
    ...
    # initial menu text
    clicked.set("Select the WBC type")
...
#Testcase 1 : If image is not uploaded for testing

    if img is None:
        out="No image uploaded"
        messagebox.showwarning("Error", "Error Message : " + str(out))
        
    else:
         messagebox.showinfo("Upload Success", "Image Uploaded Successfully for testing")


#Testcase 2 : If image is not in proper Extension

    import imghdr
    path = import_file_path
    if imghdr.what(path)=='jpeg' :
        messagebox.showinfo('Good file:', "Image is in right format : \n" +imghdr.what(path)+ "\n" + "Image Uploaded Successfully for testing")

    elif imghdr.what(path)=='jpg':
        messagebox.showinfo('Good file:', "Image is in right format : \n" + imghdr.what(
            path) + "\n" + "Image Uploaded Successfully for testing")
        ...
        messagebox.showerror("Image Format Error", "Image is with wrong extension for analysis: \n"+imghdr.what(path))
...
def result():
    ...
    if result[0][0] == 1:
        ...
        print("LYMPHOCYTE")
        out="Acute Myeloid Leukemia(AML)"
        messagebox.showinfo("Result", "Classification Result : " + str(out))
        ...
        print("NEUTROPHIL")
        out = "Chronic Myeloid Leukemia (CML)"
        messagebox.showinfo("Result", "Classification Result : " + str(out))
...
def main_account_screen():
    ...
    main_screen.title("Blood Leukemia Prediction")

    Label(text="Multiple Blood Leukemia Prediction using Deep Learning", width="300", height="5", font=("Calibri", 16)).pack()
